...
def load_att_text(e_num, path):
    ...
    att_text_emb = np.array(
        [att_text_dict[i] if i in att_text_dict else np.random.normal(mean, std, mean.shape[0]) for i in range(e_num)])
...
def load_rel_text(e_num, path):
    ...
    rel_text_emb = np.array(
        [rel_text_dict[i] if i in rel_text_dict else np.random.normal(mean, std, mean.shape[0]) for i in range(e_num)])
